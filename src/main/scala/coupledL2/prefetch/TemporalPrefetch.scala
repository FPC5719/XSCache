...
/* VIVT, Physical Data */
class TemporalPrefetch(implicit p: Parameters) extends TPModule with CacheableModule {
  ...
  println(s"[DEBUG] Elaborating non-cacheable area with hartid = ${hartid}")
  ...
  val tpTable_w_valid_wire = Wire(Bool())
  val s2_valid_wire = Wire(Bool())

  triggerDB.log(triggerPt, tpTable_w_valid_wire, "", clock, reset)
  trainDB.log(trainPt, s2_valid_wire, "", clock, reset)
  sendDB.log(sendPt, io.req.fire, "", clock, reset)

  protected def cacheable(): Unit = {
    println(s"[DEBUG] Elaborating cacheable area")
    ...
    val train_ready = resetFinish
    pending_valid := io.train.valid && train_ready && Mux(trainOnVaddr.orR, io.train.bits.vaddr.getOrElse(0.U) =/= 0.U, true.B) &&
    ...
    val s0_valid = (io.train.valid && train_ready && Mux(trainOnVaddr.orR, io.train.bits.vaddr.getOrElse(0.U) =/= 0.U, true.B) &&
    ...
    */
    ...
    val deq_ready = tpDataQFull || !do_sending
    tpDataQueue.io.deq.ready := deq_ready
    when(tpDataQueue.io.deq.valid && deq_ready) {
    ...
    val req_valid = Mux(enable, sending_valid, false.B)
    when(io.req.ready && req_valid) {
    ...
    tpTable_w_valid_wire := tpTable_w_valid
    s2_valid_wire := s2_valid
  }
}
